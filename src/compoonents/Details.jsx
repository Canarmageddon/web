...
export default function ({ display, setContentPage }) {
    ...
    }}>{route.listLocations.length < 1 ?
        "rien à afficher" :

        route.listLocations.map((step) => {
            return <div key={step.id} > {step.description}
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-triangle-fill" viewBox="0 0 16 16" onClick={() => handleClick(step.id)} >
                ...
                {currentRoute == step.id
                    && <div >{currentPoi.map((e) => <div style={{ display: "flex", flexDirection: "row" }} key={e.id}> <p>{e.description}</p>
                        <FontAwesomeIcon
                            icon={faPen}
